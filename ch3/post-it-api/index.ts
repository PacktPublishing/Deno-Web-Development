...
function seed() {
  const dataToSeed = [
    {
      title: "Deno",
      body: "Read PacktPub book",
      createdAt: new Date(),
      id: "",
    },
    {
      title: "Learning",
      body: "Write an application with Deno",
      createdAt: new Date(),
      id: "asdsa",
    },
  ];
...
async function handleRoute(req: ServerRequest) {
  ...
  switch (methodUrl) {
    case "GET /api/post-its": {
      const postItsList = Object.keys(postIts).reduce(
      ...
      return req.respond(
        { body: JSON.stringify({ postIts: postItsList }), headers },
      );
    }
    case "POST /api/post-its": {
      ...
      return req.respond(
        { status: 201, body: JSON.stringify(postIts[id]), headers },
      );
    }
    ...
    default:
      return req.respond(
        { status: 404, body: JSON.stringify({ message: "Route not found" }) },
      );
  }
}
