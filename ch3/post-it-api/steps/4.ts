...
let postIts: Record<string, PostIt> = {
  'random-uuid-1': { title: 'Read more', body: 'PacktPub books', id: 'random-uuid-1', createdAt: new Date() },
  'random-uuid-2': { title: 'Finish book', body: 'Deno Web Development', id: 'random-uuid-2', createdAt: new Date() }
}
/**
...
for await (let req of server) {
  ...
  switch (pathWithMethod) {
    case 'GET /api/post-its': {
      ...
      req.respond({ headers, body: JSON.stringify({ postIts: allPostIts }) });
      continue;
    }
    default:
